...
const SeriesOptions = {
    ...
    setup() {
        ...
        onMounted(() => {
            const el = document.querySelector('#seriesOptions')
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="seriesOptions">
        ...
        <div class="offcanvas-body p-0">
            <div v-if="loading" class="list-group list-group-flush" >
                <div class="list-group-item list-group-item-action px-4 py-3 d-flex align-items-center border-0">
            ...
            <div v-else class="list-group list-group-flush">
                ...
                <button class="list-group-item list-group-item-action px-4 py-3 d-flex align-items-center border-0 text-primary"
                    v-if="[TvStatus.Watching, TvStatus.Stopped].includes(selected.Status)" class="btn btn-primary" @click="onStatus">
                    {{ statusActionTxt }}
                </button>
                <button class="list-group-item list-group-item-action px-4 py-3 d-flex align-items-center border-0 text-danger"
                    class="btn btn-primary" @click="onAddRemSeries">
                    {{ addRemActionTxt }}
                </button>
            </div>
        </div>
